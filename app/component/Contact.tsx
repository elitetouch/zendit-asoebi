...
export const SubmitComponent=({handleSubmission, submitLoader}:handleSubmission)=>{
  //file uploads//
 const inputRef = useRef<HTMLInputElement>(null);
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      // Optional: Upload the file here or call your backend
      // uploadImage(file)
    }
  };
  return(
  <Box className=' lg:flex grid items-center lg:gap-x-[20px] gap-y-[20px] lg:w-full w-11/12 m-auto pt-[40px] lg:pt-0'>
                 <Button isLoading={submitLoader} onClick={handleSubmission} height={50} backgroundColor={'#FE9534'} roundedLeft={'full'} roundedRight={'full'} className='  rounded-l-full rounded-r-full '>
                                                <Text className='text-white '>Submit your request</Text>
                                                </Button>
                 <Button onClick={handleImageClick} height={50}  backgroundColor={'transparent'} roundedLeft={'full'} roundedRight={'full'}>
...
<path d="M2 16C2 15.4477 1.55228 15 1 15C0.447715 15 0 15.4477 0 16V17.5C0 19.7091 1.79086 21.5 4 21.5H16C18.2091 21.5 20 19.7091 20 17.5V16C20 15.4477 19.5523 15 19 15C18.4477 15 18 15.4477 18 16V17.5C18 18.6046 17.1046 19.5 16 19.5H4C2.89543 19.5 2 18.6046 2 17.5V16Z" fill="#031966"/>
      ...
      />
                                               ...
                                               <Button  as="a" href="https://example.com/yourfile.xlsx" target="_blank" rel="noopener noreferrer"   height={50}  backgroundColor={'transparent'} roundedLeft={'full'} roundedRight={'full'}>
                                                <Box className=' flex items-center gap-x-[10px]'>
                                                    <Box>
...
const handleSubmission=async()=>{
  ...
  formData.append('email', contactData.email)
   try {
    const response = await axios.post('/api/contact', formData); // Replace with your endpoint
        ...
        <Box className=' grid lg:grid-cols-2 mt-[60px] w-10/12 lg:w-10/12 m-auto'>
            <Box>
                ...
                <Box className=' text-[18px] lg:mt-[20px] mt-[10px]'>
                   ...
                   <Text className='text-[#031966] italic lg:pt-[20px] pt-[10px] lg:text-[18px] text-[18px] '>Reliable and efficient</Text>
                </Box>
            </Box>
            <Box data-aos="fade-up" className=' mt-[20px] lg:mt-0 grid gap-y-[20px]'>
                    <Contact_input names='name' label='Client’s Name' changes={handleChanges} values={contactData.name}  />
            ...
            <SubmitComponent submitLoader={submitLoader} handleSubmission={handleSubmission} />
            </Box>
        </Box>
